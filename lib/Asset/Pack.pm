...
At present, this distribution only works on C<< Perl >=5.8 >>. ( Though the files it B<generates> should work
as-is on C<5.6> )
...
=item * L<< C<Test::Differences>|Test::Differences >> is used a fair bit in tests, and it won't auto-install on 5.6. But careful selection of dependencies will get you a working install

=over 4

=item * C<< SMUELLER/Data-Dumper-2.128.tar.gz >> is newest version of Data::Dumper that works on C<5.6.2>, versions C<2.130_03> to C<2.154> presently fail.

=item * C<< OVID/Test-Differences-0.61.tar.gz >> is the newest version of Test::Differences that works on C<5.6.2>, versions C<0.61_01> to C<0.63> presently fail.

=back

=item * L<< C<Test::TempDir::Tiny>|Test::TempDir::Tiny >> is used a fail bit in tests as well, and as of C<0.005-TRIAL>, C<5.6> support is L<< still lacking|https://github.com/dagolden/Test-TempDir-Tiny/issues/9 >>, albeit support is L<< intended|https://github.com/dagolden/Test-TempDir-Tiny/commit/bbf9f899ddee1959867e1ad3a4830bea5400f1ef >>
